...
class TextRetrievalAbsEvaluator(AbsEvaluator):
    """
    ...
    def __call__(
        ...
        retriever: TextRetrievalEvalRetriever,
        reranker: Optional[TextRetrievalEvalReranker] = None,
        corpus_embd_save_dir: Optional[str] = None,
    ...
    def output_eval_results_to_markdown(eval_results_dict: dict, output_path: str, metrics: Union[List[str], str]):
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            for metric in metrics:
                f.write(f"## {metric}\n\n")
                results_df = TextRetrievalAbsEvaluator.get_results_df(metric, eval_results_dict)
